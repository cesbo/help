...
- MPTS-потоки: можно задать адрес MPTS-потока и просканировать его для добавления всех каналов по отдельности. [Подробнее](/ru/astra/receiving/mpts-via-udp)
- CI-адаптер: можно настроить дешифрование каналов с помощью внешних CI-адаптеров, таких как DigitalDevices Octopus Twin CI или TBS6900. [Подробнее](/ru/astra/receiving/external-ci)
- Адаптер SAT>IP: это замена флажка SAT>IP в предыдущих версиях. [Подробнее](/ru/astra/receiving/satip-client)
...
- `app_mem_kb` в [API состояния процесса](/ru/astra/admin-guide/process). Использование памяти процесса в килобайтах
- `video_count` и `audio_count` в [API состояния потока](/ru/astra/admin-guide/stream#get-stream-status)
